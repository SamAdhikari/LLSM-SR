SigmaUpdateRW <-
function(Aprior,Bprior,Z,nn,dd,TT,gList)
    {
    sigma = array(0,dim=c(dd,dd))
    Apost = Aprior + dd*sum(nn)/2
        ...
        for(tt in 1:TT){
            ...
            if(tt == 1){
                ...
                if(tt == TT){
                    for(kk in 1:nn[tt]){	
                        if(gList[kk+Jump]==1){
                            B1=B1+((Z[[tt]][kk,jj]-Z[[tt-1]][paste(nameList[kk]),jj])^2)/4 ##accounting for the prior
    ...
    Bpost = sum(BB) + Bprior
    sigma[1,1] =  sigma[2,2] = 1/rgamma(1,Apost,Bpost)
